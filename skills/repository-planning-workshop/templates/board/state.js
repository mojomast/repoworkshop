...
function validateManifest(manifest) {
  ...
  exact(manifest, ["schemaVersion", "manifestVersion", "generatedAt", "project", "researchBaseline", "baselineDigest", "manifestDigest", "limits", "evidence", "epics", "decisions", "blockers"], "manifest");
  ...
  array(manifest.epics, "epics", 512); const epicIds = manifest.epics.map((epic, index) => {
    exact(epic, ["id", "title", "summary", "classification", "evidenceIds", "dependsOnEpicIds", "requiredDecisionIds", "initialEnabled", "initialDisposition", "suggestedPriority", "priorityScore", "priorityBreakdown", "scope", "exclusions", "risks"], `epics[${index}]`);
    match(epic.id, EPIC_ID, "epic.id"); text(epic.title, "epic.title", 1, 120); text(epic.summary, "epic.summary", 0, 1000); if (!["present", "partial", "missing", "health", "deferred"].includes(epic.classification)) fail(`${epic.id} classification invalid`);
  ...
  array(manifest.decisions, "decisions", 256); const decisionIds = manifest.decisions.map((decision, index) => {
    ...
    array(decision.options, `${decision.id}.options`, 4, 2); const options = decision.options.map((option, optionIndex) => { exact(option, ["id", "label", "implementationShape", "benefits", "costsAndRisks", "migrationAndOperations", "evidenceIds"], `${decision.id}.options[${optionIndex}]`); match(option.id, OPTION_ID, "option.id"); if (!option.id.startsWith(`${decision.id}-OPT-`)) fail(`${option.id} does not belong to ${decision.id}`); text(option.label, "option.label", 1, 120); text(option.implementationShape, "option.implementationShape", 1, 1000); for (const key of ["benefits", "costsAndRisks"]) { array(option[key], `option.${key}`, 16, 1); option[key].forEach((item, i) => text(item, `option.${key}[${i}]`, 1, 1000)); } text(option.migrationAndOperations, "option.migrationAndOperations", 0, 1000); array(option.evidenceIds, "option.evidenceIds", 64, 1); option.evidenceIds.forEach((id) => { if (!evidenceIds.includes(id)) fail(`${option.id} unknown evidence`); }); return option.id; }); unique(options, `${decision.id} option IDs`); if (!options.includes(decision.recommendedOptionId)) fail(`${decision.id} recommended option invalid`); text(decision.recommendationRationale, "recommendationRationale", 1, 1000); exact(decision.customAnswer, ["allowed", "maxLength", "validation"], `${decision.id}.customAnswer`); if (typeof decision.customAnswer.allowed !== "boolean") fail("customAnswer.allowed invalid"); safeInteger(decision.customAnswer.maxLength, "customAnswer.maxLength", 1, 2000); if (!["nonblank-trimmed", "single-line", "multiline"].includes(decision.customAnswer.validation)) fail("customAnswer.validation invalid"); return decision.id;
  }); unique(decisionIds, "decision IDs"); dag(manifest.decisions, "dependsOnDecisionIds", "decision"); for (const epic of manifest.epics) epic.requiredDecisionIds.forEach((id) => { if (!decisionIds.includes(id)) fail(`${epic.id} has unknown decision ${id}`); });
...
function validateState(state, manifest, { allowSynthesized = false } = {}) {
  validateScalars(state, "state"); exact(state, ["schemaVersion", "baselineDigest", "manifestDigest", "stateDigest", "revision", "updatedAt", "ready", "epics", "decisions", "blockers", "overallNotes"], "state"); if (state.schemaVersion !== 1 || state.baselineDigest !== manifest.baselineDigest || state.manifestDigest !== manifest.manifestDigest) fail("state schema or authority digest mismatch"); digest(state.stateDigest, "state.stateDigest"); if (state.stateDigest !== stateDigest(state)) fail("state.stateDigest mismatch"); safeInteger(state.revision, "state.revision", allowSynthesized ? 0 : 1); if (state.revision === 0 && !allowSynthesized) fail("persisted state revision must be at least 1"); utc(state.updatedAt, "state.updatedAt"); if (typeof state.ready !== "boolean") fail("state.ready invalid");
  array(state.epics, "state.epics", manifest.epics.length, manifest.epics.length); state.epics.forEach((answer, index) => { exact(answer, ["id", "enabled", "disposition", "dispositionReason", "notes"], `state.epics[${index}]`); if (answer.id !== manifest.epics[index].id) fail("state epic IDs/order must exactly match manifest"); if (typeof answer.enabled !== "boolean" || !["Build", "Remove", "Defer", "Need decision"].includes(answer.disposition)) fail(`${answer.id} controls invalid`); text(answer.dispositionReason, `${answer.id}.dispositionReason`, 0, 1000); text(answer.notes, `${answer.id}.notes`, 0, manifest.limits.epicNotesMax); });
  array(state.decisions, "state.decisions", manifest.decisions.length, manifest.decisions.length); state.decisions.forEach((answer, index) => { const source = manifest.decisions[index]; exact(answer, ["id", "selectedOptionId", "customAnswer", "notes"], `state.decisions[${index}]`); if (answer.id !== source.id) fail("state decision IDs/order must exactly match manifest"); if (answer.selectedOptionId !== null && !source.options.some((option) => option.id === answer.selectedOptionId)) fail(`${answer.id} selected option invalid`); if (answer.customAnswer !== null) { if (!source.customAnswer.allowed) fail(`${answer.id} custom answer forbidden`); text(answer.customAnswer, `${answer.id}.customAnswer`, 0, Math.min(source.customAnswer.maxLength, manifest.limits.decisionCustomMax)); if (source.customAnswer.validation === "single-line" && /[\r\n]/.test(answer.customAnswer)) fail(`${answer.id} custom answer must be single-line`); } text(answer.notes, `${answer.id}.notes`, 0, 1000); });
...
function approvedSelectionSnapshot(state, manifest) { validateState(state, manifest); if (!state.ready) fail("state is not ready for retrieval"); const snapshot = { schemaVersion: 1, manifestDigest: state.manifestDigest, baselineDigest: state.baselineDigest, sourceStateRevision: state.revision, sourceStateDigest: state.stateDigest, epics: structuredClone(state.epics), decisions: structuredClone(state.decisions), blockers: structuredClone(state.blockers), overallNotes: state.overallNotes, snapshotDigest: "" }; snapshot.snapshotDigest = hash(omit(snapshot, "snapshotDigest")); return snapshot; }
